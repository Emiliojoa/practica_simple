...
export const deleteText = async (req, res) => {
    try {
        const { id } = req.params
        const deleteText = await textModel.findByIdAndDelete(id)
        return res.status(200).json({ msg: `Texto eliminado ${deleteText}` })
    } catch (error) {
        return res.status(400).json("Error al eliminar el texto")
    }
}
